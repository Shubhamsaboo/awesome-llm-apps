...
try:
    ...
    print(
        f"CSV dataset loaded successfully: {csv_file_path}. Number of rows: {len(df)}"
    )
    ...
    for _, row in df.iterrows():
        ...
        card_meanings[card_name] = {
            ...
            "reversed": str(row["reversed"]).strip()
            if pd.notna(row["reversed"])
            else "",
            "symbolism": str(row["symbolism"]).strip()
            if pd.notna(row["symbolism"])
            else "",
        }
...
analyzer = (
    RunnableParallel(cards=lambda x: x["cards"], context=lambda x: x["context"])
...
num_cards = st.selectbox(
    "🃏 Select the number of cards for your spread (3 for a more focused answer, 7 for a more general overview).)",
    [3, 5, 7],
)
context_question = st.text_area(
    "✍️ Please enter your context or your question here. You can speak in natural language.",
    height=100,
)
            ...
            for i, card_info in enumerate(drawn_cards_list):
                with cols[i]:
                    ...
                    try:
                        ...
                        st.info(
                            f"Symbol: {card_info['name']} {reversed_label} (Image not found at {image_path})"
                        )

            st.markdown("---")
            with st.spinner("🔮 Unveiling the meanings..."):
                analysis_result = analyzer.invoke(
                    {"cards": drawn_cards_list, "context": context_question}
                )
