...
def main():
    ...
    print(f"[OK] Status: {outcome_1.status}")
    print(f"     Attempt: {outcome_1.attempt_number}")
    if outcome_1.status == OutcomeStatus.SUCCESS.value or outcome_1.status == "SUCCESS":
        print(f"\n[OK] Refactored code (first 300 chars):")
        print(outcome_1.code_output[:300] if outcome_1.code_output else "N/A")
        print(f"\n[!] Key insight: {outcome_1.key_insight[:150] if outcome_1.key_insight else 'N/A'}")
    else:
        print(f"\n[ERROR] Error: {outcome_1.error_message}")
    ...
    print(f"[OK] Status: {outcome_2.status}")
    print(f"     Attempt: {outcome_2.attempt_number}")
    if outcome_2.status == OutcomeStatus.SUCCESS.value or outcome_2.status == "SUCCESS":
        print(f"\n[OK] Refactored code (first 300 chars):")
        print(outcome_2.code_output[:300] if outcome_2.code_output else "N/A")
        print(f"\n[!] Key insight: {outcome_2.key_insight[:150] if outcome_2.key_insight else 'N/A'}")
    else:
        print(f"\n[ERROR] Error: {outcome_2.error_message}")
    ...
    print(f"[!] Strategy adaptations: {summary['strategy_adaptations']}")
    
    if summary['adaptations_made']:
        print(f"\n[*] Adaptations made:")
        for adaptation in summary['adaptations_made'][:3]:
            print(f"   - {adaptation[:80]}...")
