    ...
    try:
        ...
        if transcript is None and available_languages:
            try:
                fetched_transcript = api.fetch(
                    video_id, languages=[available_languages[0]]
                )
                transcript = list(fetched_transcript)
                st.success(
                    f"Successfully fetched transcript in language: {available_languages[0]}"
                )
        ...
        if transcript:
            ...
            return (
                "Unknown",
                transcript_text,
            )  # Title is set to "Unknown" since we're not fetching it
        else:
            return "Unknown", "No transcript available for this video."
...
if openai_access_token:
        ...
        with st.spinner("🔍 Checking video and fetching transcript..."):
            try:
                ...
                if (
                    ...
                    and transcript != "Invalid YouTube URL provided."
                ):
                    with st.spinner("🧠 Adding to knowledge base..."):
                        ...
                        st.session_state.app.add(
                            transcript,
                            data_type="text",
                            metadata={"title": title, "url": video_url},
                        )
                        ...
                        st.success(
                            "✅ Added video to knowledge base! You can now ask questions about it."
                        )
                        st.info(
                            f"📊 Transcript contains {st.session_state.word_count} words"
                        )
                else:
    ...
    if st.session_state.transcript_loaded and st.session_state.current_video_url:
        ...
        with col1:
            ...
            st.info(
                f"📊 Transcript: {st.session_state.word_count} words loaded in memory"
            )
        with col2:
            if st.button(
                "🗑️ Clear Video", help="Remove current video and load a new one"
            ):
                st.session_state.transcript_loaded = False
                st.session_state.current_video_url = None
                st.session_state.transcript_text = None
    ...
    if prompt:
        if st.session_state.transcript_loaded and st.session_state.app:
            ...
            st.warning(
                "⚠️ Please add a valid video with transcripts first before asking questions."
            )
