...
def test_session_state_logic():
    ...
    mock_transcript = (
        "This is a mock transcript for testing purposes. It contains multiple words."
    )

    # Simulate the logic from the app
    if (
        video_url_1 != session_state.current_video_url
        or not session_state.transcript_loaded
    ):
    ...
    if (
        video_url_2 != session_state.current_video_url
        or not session_state.transcript_loaded
    ):
...
if __name__ == "__main__":
    ...
    print("   1. ✅ Transcript loads only once per video URL")
    print("   2. ✅ Chat history is preserved for the same video")
    print("   3. ✅ Chat history is cleared when loading a new video")
    print("   4. ✅ No redundant API calls for the same URL")
    print("   5. ✅ Session state properly manages video data")
