...
class TourManager:
    ...
    async def run(self, query: str, interests: list, duration: str) -> None:
        ...
        with trace("Tour Research trace", trace_id=trace_id):
            ...
            if "Architecture" in interests:
                research_results["architecture"] = await self._get_architecture(
                    query, interests, words_per_section
                )

            if "History" in interests:
                research_results["history"] = await self._get_history(
                    query, interests, words_per_section
                )

            if "Culinary" in interests:
                research_results["culinary"] = await self._get_culinary(
                    query, interests, words_per_section
                )

            if "Culture" in interests:
                research_results["culture"] = await self._get_culture(
                    query, interests, words_per_section
                )

            # Get final tour with only selected interests
            final_tour = await self._get_final_tour(
                query, interests, duration, research_results
            )
    ...
    async def _get_plan(self, query: str, interests: list, duration: str) -> Planner:
        ...
        result = await Runner.run(
            planner_agent,
            "Query: {} Interests: {} Duration: {}".format(
                query, ", ".join(interests), duration
            ),
        )
        self.printer.update_item(
            "Planner",
    ...
    async def _get_history(
        self, query: str, interests: list, word_limit: int
    ) -> History:
    ...
    async def _get_final_tour(
        self, query: str, interests: list, duration: float, research_results: dict
    ):
