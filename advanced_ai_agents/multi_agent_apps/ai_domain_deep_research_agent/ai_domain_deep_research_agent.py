...
if together_api_key and composio_api_key:
    ...
    with col1:
        topic = st.text_input(
            "What topic would you like to research?", placeholder="American Tariffs"
        )
    with col2:
    ...
    if (
        topic
        and domain
        and st.button("Generate Research Questions", key="generate_questions")
    ):
        # Generate questions
    ...
    if st.session_state.questions and st.button("Start Research", key="start_research"):
        ...
        if st.button("Compile Final Report", key="compile_report"):
            report_content = compile_report(
                llm, composio_tools, topic, domain, question_answers
            )

            # Display the report content
            st.header("Final Report")
            st.success(
                "Your report has been compiled and a Google Doc has been created."
            )
    ...
    if (
        len(st.session_state.question_answers) > 0
        and not st.session_state.research_complete
    ):
    ...
    st.warning(
        "⚠️ Please enter your Together AI and Composio API keys in the sidebar to get started."
    )
    ...
    with col2:
        ...
        st.write(
            "The AI generates specific research questions to explore your topic in depth."
        )

    with col3:
        st.subheader("3️⃣ Compile Report")
        st.write(
            "Research findings are compiled into a professional report and saved to Google Docs."
        )
