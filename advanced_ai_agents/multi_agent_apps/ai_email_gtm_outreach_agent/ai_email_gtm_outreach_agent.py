...
def run_pipeline(
    ...
    companies = run_company_finder(
        company_agent, target_desc, offering_desc, max_companies=num_companies
    )
    contacts_data = (
        run_contact_finder(contact_agent, companies, target_desc, offering_desc)
        if companies
        else []
    )
...
def main() -> None:
    ...
    with col1:
        target_desc = st.text_area(
            "Target companies (industry, size, region, tech, etc.)", height=100
        )
        offering_desc = st.text_area(
            "Your product/service offering (1-3 sentences)", height=100
        )
    with col2:
    ...
    if st.button("Start Outreach", type="primary"):
        ...
        if not openai_key or not exa_key:
            ...
            try:
                ...
                research_data = (
                    run_research(research_agent, companies) if companies else []
                )
    ...
    if results:
        ...
        if emails:
            for i, e in enumerate(emails, 1):
                with st.expander(
                    f"{i}. {e.get('company', '')} → {e.get('contact', '')}"
                ):
