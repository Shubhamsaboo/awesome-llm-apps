...
class EditLandingPageInput(BaseModel):
    artifact_filename: str = Field(
        ..., description="The filename of the landing page artifact to edit."
    )
    prompt: str = Field(
        ..., description="Detailed description of UI/UX improvements to apply."
    )
    asset_name: str = Field(
        default=None, description="Optional: specify asset name for the new version."
    )
...
class GenerateImprovedLandingPageInput(BaseModel):
    prompt: str = Field(
        ...,
        description="A detailed description of the improved landing page based on feedback.",
    )
    aspect_ratio: str = Field(
        default="16:9", description="The desired aspect ratio. Default is 16:9."
    )
    asset_name: str = Field(
        default="landing_page_improved",
        description="Base name for the improved design.",
    )
    reference_image: str = Field(
        default=None,
        description="Optional: filename of the original landing page to use as reference.",
    )
...
"""
        ...
        logger.info(
            f"Editing landing page with artifact filename: {edited_artifact_filename} (version {version})"
        )
        ...
        for chunk in client.models.generate_content_stream(
            ...
            if (
                ...
                try:
                    ...
                    tool_context.state["last_edited_landing_page"] = (
                        edited_artifact_filename
                    )
                    tool_context.state["current_asset_name"] = asset_name

                    logger.info(
                        f"Saved edited landing page as artifact '{edited_artifact_filename}' (version {version})"
                    )
...
async def generate_improved_landing_page(
    ...
    try:
        ...
        if inputs.reference_image:
            try:
                reference_part = await load_landing_page_image(
                    tool_context, inputs.reference_image
                )
                if reference_part:
                    logger.info(
                        f"Using reference image artifact: {inputs.reference_image}"
                    )
            except Exception as e:
                logger.warning(
                    f"Could not load reference image, proceeding without it: {e}"
                )
...
"""
        ...
        logger.info(
            f"Generating improved landing page with filename: {artifact_filename} (version {version})"
        )
        ...
        for chunk in client.models.generate_content_stream(
            ...
            if (
                ...
                try:
                    ...
                    tool_context.state["last_generated_landing_page"] = (
                        artifact_filename
                    )
                    tool_context.state["current_asset_name"] = inputs.asset_name

                    logger.info(
                        f"Saved improved landing page as artifact '{artifact_filename}' (version {version})"
                    )
