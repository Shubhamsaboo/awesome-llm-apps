...
def setup_sidebar() -> None:
    with st.sidebar:
        ...
        st.session_state.e2b_key = st.text_input(
            "E2B API Key", value=st.session_state.e2b_key, type="password"
        )
...
def run_code_in_sandbox(code: str) -> Dict[str, Any]:
    ...
    return {"logs": execution.logs, "files": st.session_state.sandbox.files.list("/")}
...
def process_image_with_gemini(vision_agent: Agent, image: Image) -> str:
    ...
    try:
        ...
        response: RunOutput = vision_agent.run(prompt, images=[agno_image])
        return response.content
    except Exception as e:
        st.error(f"Error processing image: {str(e)}")
        return (
            "Failed to process the image. Please try again or use text input instead."
        )
    finally:
...
def main() -> None:
    ...
    uploaded_image = st.file_uploader(
        "Upload an image of your coding problem (optional)", type=["png", "jpg", "jpeg"]
    )
    ...
    if st.button("Generate & Execute Solution", type="primary"):
        if uploaded_image and not user_query:
            ...
            st.warning(
                "Please provide either an image or text description of your coding problem."
            )
            return
