...
def process_image_with_gemini(vision_agent: Agent, image: Image) -> str:
    ...
    try:
        ...
        response = vision_agent.run(
            prompt, images=[{"filepath": temp_path}]  # Use filepath instead of content
        )
...
def main() -> None:
    ...
    if not (
        st.session_state.openai_key and st.session_state.gemini_key and st.session_state.e2b_key
    ):
