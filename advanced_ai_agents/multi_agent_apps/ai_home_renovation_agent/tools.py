...
def create_versioned_filename(
    asset_name: str, version: int, file_extension: str = "png"
) -> str:
...
def get_asset_versions_info(tool_context: ToolContext) -> str:
    ...
    for asset_name, current_version in asset_versions.items():
        ...
        latest_filename = tool_context.state.get("asset_filenames", {}).get(
            asset_name, "Unknown"
        )
        info_lines.append(
...
class EditRenovationRenderingInput(BaseModel):
    artifact_filename: str = Field(
        ..., description="The filename of the rendering artifact to edit."
    )
    prompt: str = Field(
        ...,
...
async def generate_renovation_rendering(
    tool_context: ToolContext, inputs: GenerateRenovationRenderingInput
) -> str:
    """
    ...
    try:
        ...
        if inputs.current_room_photo:
            current_photo_part = await load_reference_image(
                tool_context, inputs.current_room_photo
            )
        ...
        if inputs.inspiration_image:
            ...
            if insp_filename:
                inspiration_part = await load_reference_image(
                    tool_context, insp_filename
                )
                if inspiration_part:
        ...
        rewritten_prompt_response = client.models.generate_content(
            model="gemini-2.5-flash", contents=base_rewrite_prompt
        )
        ...
        logger.info(
            f"Generating rendering with artifact filename: {artifact_filename} (version {version})"
        )
            ...
            if (
                ...
                try:
                    ...
                    update_asset_version(
                        tool_context, inputs.asset_name, version, artifact_filename
                    )
                    ...
                    logger.info(
                        f"Saved rendering as artifact '{artifact_filename}' (version {version})"
                    )
        ...
        return (
            "No rendering was generated. Please try again with a more detailed prompt."
        )
...
async def edit_renovation_rendering(
    tool_context: ToolContext, inputs: EditRenovationRenderingInput
) -> str:
    """
    ...
    try:
        ...
        try:
            loaded_image_part = await tool_context.load_artifact(
                inputs.artifact_filename
            )
            if not loaded_image_part:
                return (
                    f"❌ Could not find rendering artifact: {inputs.artifact_filename}"
                )
        except Exception as e:
        ...
        if inputs.reference_image_filename:
            ...
            if ref_filename:
                reference_image_part = await load_reference_image(
                    tool_context, ref_filename
                )
        ...
        logger.info(
            f"Editing rendering with artifact filename: {edited_artifact_filename} (version {version})"
        )
        ...
        for chunk in client.models.generate_content_stream(
            ...
            if (
                ...
                try:
                    ...
                    version = await tool_context.save_artifact(
                        filename=edited_artifact_filename, artifact=edited_image_part
                    )

                    # Update version tracking
                    update_asset_version(
                        tool_context, asset_name, version, edited_artifact_filename
                    )

                    # Store in session state
                    tool_context.state["last_generated_rendering"] = (
                        edited_artifact_filename
                    )
                    tool_context.state["current_asset_name"] = asset_name

                    logger.info(
                        f"Saved edited rendering as artifact '{edited_artifact_filename}' (version {version})"
                    )
...
async def save_uploaded_image_as_artifact(
    ...
    try:
        ...
        # Save as artifact
        await tool_context.save_artifact(filename=artifact_name, artifact=image_data)
