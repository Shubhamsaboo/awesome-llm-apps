...
class EmergencyFund(BaseModel):
    recommended_amount: float = Field(
        ..., description="Recommended emergency fund size"
    )
    current_amount: Optional[float] = Field(
        None, description="Current emergency fund (if any)"
    )
...
class SavingsRecommendation(BaseModel):
    ...
    rationale: Optional[str] = Field(
        None, description="Explanation for this recommendation"
    )
...
class SavingsStrategy(BaseModel):
    emergency_fund: EmergencyFund = Field(
        ..., description="Emergency fund recommendation"
    )
    recommendations: List[SavingsRecommendation] = Field(
        ..., description="Savings allocation recommendations"
    )
...
class PayoffPlan(BaseModel):
    ...
    monthly_payment: Optional[float] = Field(
        None, description="Recommended monthly payment"
    )
...
IMPORTANT: Store your final plan in state['debt_reduction'] and ensure it aligns with the previous analyses.""",
        ...
        try:
            ...
            async for event in self.runner.run_async(
                ...
                if (
                    ...
                    and event.author == self.coordinator_agent.name
                ):
                    break
            ...
            for key in ["budget_analysis", "savings_strategy", "debt_reduction"]:
                ...
                results[key] = (
                    parse_json_safely(value, default_results[key])
                    if value
                    else default_results[key]
                )
    ...
    def _create_default_results(self, financial_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "debt_reduction": {
                "total_debt": sum(
                    debt.get("amount", 0) for debt in financial_data.get("debts", [])
                ),
                "debts": financial_data.get("debts", []),
                "payoff_plans": {
                    "avalanche": {
                        "total_interest": sum(
                            debt.get("amount", 0)
                            for debt in financial_data.get("debts", [])
                        )
                        * 0.2,
                        "months_to_payoff": 24,
                        "monthly_payment": sum(
...
def display_savings_strategy(strategy: Dict[str, Any]):
    ...
    if "emergency_fund" in strategy:
        ...
        if "current_amount" in ef and "recommended_amount" in ef:
            ...
            st.markdown(
                f"${ef['current_amount']:.2f} of ${ef['recommended_amount']:.2f}"
            )
    ...
    if "payoff_plans" in plan:
        ...
        with tabs[0]:
            ...
            if "avalanche" in plan["payoff_plans"]:
                ...
                st.markdown(
                    f"**Total Interest Paid**: ${avalanche['total_interest']:.2f}"
                )
                st.markdown(
                    f"**Time to Debt Freedom**: {avalanche['months_to_payoff']} months"
                )

                if "monthly_payment" in avalanche:
                    st.markdown(
                        f"**Recommended Monthly Payment**: ${avalanche['monthly_payment']:.2f}"
                    )
        ...
        with tabs[1]:
            ...
            if "snowball" in plan["payoff_plans"]:
                ...
                st.markdown(
                    f"**Total Interest Paid**: ${snowball['total_interest']:.2f}"
                )
                st.markdown(
                    f"**Time to Debt Freedom**: {snowball['months_to_payoff']} months"
                )

                if "monthly_payment" in snowball:
                    st.markdown(
                        f"**Recommended Monthly Payment**: ${snowball['monthly_payment']:.2f}"
                    )

        with tabs[2]:
            st.markdown("### Method Comparison")
            if (
                "avalanche" in plan["payoff_plans"]
                and "snowball" in plan["payoff_plans"]
            ):
                avalanche = plan["payoff_plans"]["avalanche"]
                snowball = plan["payoff_plans"]["snowball"]
                ...
                fig = go.Figure(
                    data=[
                        go.Bar(
                            ...
                            y=comparison_df["Total Interest"],
                        ),
                        go.Bar(
                            name="Months to Payoff",
                            x=comparison_df["Method"],
                            y=comparison_df["Months to Payoff"],
                        ),
                    ]
                )
                fig.update_layout(
                    barmode="group", title="Debt Payoff Method Comparison"
                )
                st.plotly_chart(fig)
...
2024-01-03,Transportation,45.00"""
    ...
    st.info(
        "This tool analyzes your financial data and provides tailored recommendations for budgeting, savings, and debt management using multiple specialized AI agents."
    )
    st.divider()
    ...
    with input_tab:
                ...
                if transaction_file is not None:
                    ...
                    if is_valid:
                        try:
                            ...
                            st.success(
                                "✅ Transaction file uploaded and validated successfully!"
                            )
                        except Exception as e:
        ...
        with st.container():
            ...
            st.info(
                "Enter your debts to get personalized payoff strategies using both avalanche and snowball methods."
            )
            ...
            if num_debts > 0:
                ...
                for i in range(num_debts):
                    ...
                    with cols[col_idx]:
                        ...
                        if (
                            col_idx == 2 or i == num_debts - 1
                        ):  # Add spacing after every 3 debts or last debt
                            st.markdown("---")
        ...
        if analyze_button:
            if expense_option == "Upload CSV Transactions" and transactions_df is None:
                st.error(
                    "Please upload a valid transaction CSV file or choose manual entry."
                )
                return
            if use_manual_expenses and (
                not manual_expenses or not any(manual_expenses.values())
            ):
            ...
            with st.spinner("🤖 AI agents are analyzing your financial data..."):
                ...
                try:
                    results = asyncio.run(
                        finance_system.analyze_finances(financial_data)
                    )
                    ...
                    with tabs[1]:
                        ...
                        if (
                            "savings_strategy" in results
                            and results["savings_strategy"]
                        ):
