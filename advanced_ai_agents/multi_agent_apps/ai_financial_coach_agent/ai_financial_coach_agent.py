...
IMPORTANT: Store your final plan in state['debt_reduction'] and ensure it aligns with the previous analyses.""",
        ...
        self.runner = Runner(
            agent=self.coordinator_agent, app_name=APP_NAME, session_service=self.session_service
        )
...
def display_budget_analysis(analysis: Dict[str, Any]):
    ...
    if "total_expenses" in analysis:
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Bar(x=["Income", "Expenses"], y=[income, expenses], marker_color=["green", "red"])
        )
        fig.update_layout(title="Monthly Income vs. Expenses")
        st.plotly_chart(fig)

        st.metric(
            "Monthly Surplus/Deficit", f"${surplus_deficit:.2f}", delta=f"{surplus_deficit:.2f}"
        )
            ...
            if expense_option == "📤 Upload CSV Transactions":
                ...
                if any(manual_expenses.values()):
                    ...
                    if not manual_df_disp.empty:
                        ...
                        with col2:
                            st.metric(
                                "Total Monthly Expenses", f"${manual_df_disp['Amount'].sum():,.2f}"
                            )
