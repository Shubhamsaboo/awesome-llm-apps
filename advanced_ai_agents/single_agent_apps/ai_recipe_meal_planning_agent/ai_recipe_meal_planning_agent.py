    ...
    try:
        ...
        for recipe in recipes[:3]:
            ...
            detail_response = requests.get(
                detail_url, params={"apiKey": SPOONACULAR_API_KEY}, timeout=10
            )
                ...
                detailed_recipes.append(
                    {
                        ...
                        "used_ingredients": [
                            i["name"] for i in recipe["usedIngredients"]
                        ],
                        "missing_ingredients": [
                            i["name"] for i in recipe["missedIngredients"]
                        ],
                        "instructions": detail_data.get(
                            "instructions", "Instructions not available"
                        ),
                    }
                )

        return {"recipes": detailed_recipes, "total_found": len(recipes)}
    except:
...
def estimate_costs(ingredients: List[str], servings: int = 4) -> Dict:
    ...
    for ingredient in ingredients:
        ...
        for key, price in prices.items():
            if key in ingredient_lower or any(
                word in ingredient_lower for word in key.split()
            ):
                cost = price
                break
        ...
        cost_breakdown.append(
            {"name": ingredient.title(), "cost": round(adjusted_cost, 2)}
        )
...
def main():
    st.set_page_config(
        page_title="AI Meal Planning Agent", page_icon="🍽️", layout="wide"
    )
...
What would you like to explore? 🍽️""",
    ...
    if user_input := st.chat_input(
        "Ask about recipes, nutrition, meal planning, or costs..."
    ):
