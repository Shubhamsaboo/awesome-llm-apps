...
class Agent:
    ...
    def reason(self):
        ...
        logger.info(
            colored(f"💭: Thought: {agent_data.thought}", color="light_magenta", attrs=["bold"])
        )
    ...
    def action(self):
        ...
        if isinstance(last_message, HumanMessage):
            self.agent_state.messages[-1] = HumanMessage(
                content=Prompt.previous_observation_prompt(self.agent_state.previous_observation)
            )
        ...
        prompt = Prompt.observation_prompt(
            agent_step=self.agent_step, tool_result=tool_result, desktop_state=desktop_state
        )
        human_message = (
        ...
        self.agent_state.update_state(
            agent_data=None, observation=observation, messages=[ai_message, human_message]
        )
    ...
    def answer(self):
        ...
        logger.info(
            colored(f"📜: Final Answer: {tool_result.content}", color="cyan", attrs=["bold"])
        )
        self.agent_state.update_state(
            agent_data=None, observation=None, result=tool_result.content, messages=[ai_message]
        )
    ...
    def invoke(self, query: str):
        ...
        system_message = SystemMessage(
            content=Prompt.system_prompt(
                instructions=self.instructions, tools_prompt=tools_prompt, max_steps=max_steps
            )
        )
        human_message = (
            image_message(prompt=prompt, image=desktop_state.screenshot)
        ...
        while True:
            ...
            if self.agent_state.consecutive_failures >= 3:
                ...
                return AgentResult(
                    is_done=False, content=None, error="Consecutive failures exceeded limit."
                )
