...
class Agent:
    ...
    def action(self):
        ...
        if isinstance(last_message, HumanMessage):
            self.agent_state.messages[-1] = HumanMessage(
                content=Prompt.previous_observation_prompt(
                    self.agent_state.previous_observation
                )
            )
        ai_message = AIMessage(
            content=Prompt.action_prompt(agent_data=self.agent_state.agent_data)
        )
        name = self.agent_state.agent_data.action.name
        params = self.agent_state.agent_data.action.params
        logger.info(
            colored(
                f"🔧: Action: {name}({', '.join(f'{k}={v}' for k, v in params.items())})",
                color="blue",
                attrs=["bold"],
            )
        )
        tool_result = self.registry.execute(
            tool_name=name, desktop=self.desktop, **params
        )
        observation = (
            tool_result.content if tool_result.is_success else tool_result.error
        )
        logger.info(
            colored(f"🔭: Observation: {observation}", color="green", attrs=["bold"])
        )
        ...
        human_message = (
            image_message(prompt=prompt, image=desktop_state.screenshot)
    ...
    def answer(self):
        ...
        ai_message = AIMessage(
            content=Prompt.answer_prompt(
                agent_data=self.agent_state.agent_data, tool_result=tool_result
            )
        )
        logger.info(
            colored(
                f"📜: Final Answer: {tool_result.content}", color="cyan", attrs=["bold"]
            )
        )
        self.agent_state.update_state(
            agent_data=None,
    ...
    def invoke(self, query: str):
        ...
        while True:
            if self.agent_step.is_last_step():
                ...
                return AgentResult(
                    is_done=False, content=None, error="Maximum steps reached."
                )
            self.reason()
            if self.agent_state.is_done():
                self.answer()
                return AgentResult(
                    is_done=True, content=self.agent_state.result, error=None
                )
            self.action()
            if self.agent_state.consecutive_failures >= 3:
                logger.warning(
                    "Consecutive failures exceeded limit, stopping execution."
                )
                return AgentResult(
                    is_done=False,
                    content=None,
                    error="Consecutive failures exceeded limit.",
                )
