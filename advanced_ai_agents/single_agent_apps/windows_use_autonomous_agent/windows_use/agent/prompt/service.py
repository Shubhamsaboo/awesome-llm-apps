...
class Prompt:
    @staticmethod
    def system_prompt(
        tools_prompt: str, max_steps: int, instructions: list[str] = []
    ) -> str:
        width, height = pg.size()
        template = PromptTemplate.from_file(
            files("windows_use.agent.prompt").joinpath("system.md")
        )
    ...
    def action_prompt(agent_data: AgentData) -> str:
        template = PromptTemplate.from_file(
            files("windows_use.agent.prompt").joinpath("action.md")
        )
        return template.format(
            **{
    ...
    def observation_prompt(
        agent_step: AgentStep, tool_result: ToolResult, desktop_state: DesktopState
    ) -> str:
        cursor_position = pg.position()
        tree_state = desktop_state.tree_state
        template = PromptTemplate.from_file(
            files("windows_use.agent.prompt").joinpath("observation.md")
        )
        return template.format(
            **{
                ...
                "interactive_elements": tree_state.interactive_elements_to_string()
                or "No interactive elements found",
                "informative_elements": tree_state.informative_elements_to_string()
                or "No informative elements found",
                "scrollable_elements": tree_state.scrollable_elements_to_string()
                or "No scrollable elements found",
            }
        )
    ...
    def answer_prompt(agent_data: AgentData, tool_result: ToolResult):
        template = PromptTemplate.from_file(
            files("windows_use.agent.prompt").joinpath("answer.md")
        )
        return template.format(
            **{
                "evaluate": agent_data.evaluate,
