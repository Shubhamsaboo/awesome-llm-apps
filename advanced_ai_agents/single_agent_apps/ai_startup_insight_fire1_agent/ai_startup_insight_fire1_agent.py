...
st.set_page_config(page_title="Startup Info Extraction", page_icon="🔍", layout="wide")
...
with st.sidebar:
    ...
    st.markdown("### About")
    st.markdown(
        "This tool extracts company information from websites using Firecrawl's FIRE-1 agent and provides AI-powered business analysis."
    )
...
st.markdown(
    "Provide one or more company website URLs (one per line) to extract information."
)
    ...
    if not website_urls.strip():
        ...
        try:
            with st.spinner("Extracting information from website..."):
                ...
                if not urls:
                    st.error(
                        "No valid URLs found after parsing. Please check your input."
                    )
                elif not openai_api_key:
                    st.warning(
                        "Please provide an OpenAI API key in the sidebar to get AI analysis."
                    )
                else:
                    ...
                    for i, (url, tab) in enumerate(zip(urls, tabs)):
                        with tab:
                            ...
                            with st.spinner(
                                f"FIRE agent is extracting information from {url}..."
                            ):
                                try:
                                    ...
                                    if data and data.get("data"):
                                        ...
                                        if openai_api_key:
                                            with st.spinner(
                                                ...
                                                agent_response: RunOutput = agno_agent.run(
                                                    f"Analyze this company data and provide insights: {json.dumps(company_data)}"
                                                )
                                        ...
                                        with st.expander("ℹ️ Processing Details"):
                                            ...
                                            st.markdown(
                                                "- 🔍 Scanned website content and structure"
                                            )
                                            st.markdown(
                                                "- 🖱️ Interacted with necessary page elements"
                                            )
                                            st.markdown(
                                                "- 📊 Extracted and structured data according to schema"
                                            )
                                            st.markdown(
                                                "- 🧠 Applied AI reasoning to identify relevant information"
                                            )
                                            ...
                                            if "expiresAt" in data:
                                                st.markdown(
                                                    f"**Data Expires:** {data['expiresAt']}"
                                                )
                                    else:
                                        st.error(
                                            f"No data was extracted from {url}. The website might be inaccessible, or the content structure may not match the expected format."
                                        )
