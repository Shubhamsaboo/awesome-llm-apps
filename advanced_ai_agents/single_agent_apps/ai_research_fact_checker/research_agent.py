#!/usr/bin/env python3
"""
Research Agent with Adversarial Fact-Checking
=============================================
Researches any topic by fetching real web sources, synthesizes a report with
inline citations, then adversarially fact-checks every claim — flagging
unsupported assertions and missing sources.

Pipeline: topic → research plan → fetch sources → write report → fact-check
All research sessions are logged to SQLite (~/.research_agent/).
...
MAX_FETCH = 50_000  # bytes per URL
...
def fetch_url(url: str) -> str:
    try:
        with httpx.Client(timeout=15, follow_redirects=True) as h:
            r = h.get(url, headers={"User-Agent": "research-agent/1.0"})
            r.raise_for_status()
...
{"topic_refined":"<clear restatement of the research question>",
 "search_queries":[{"index":0,"query":"<search query or URL to fetch>","rationale":"<why this source matters>"}],
...
SEARCH_SYS = """You are a research assistant. Given a search query about a topic,
generate 1-2 real, publicly accessible URLs that would contain relevant information.
Return ONLY valid JSON: {"urls":["https://..."]}
Prefer: Wikipedia, official documentation, .gov/.edu sites, major news outlets, arxiv.org.
Do NOT invent URLs — only suggest URLs you are confident actually exist."""

def gather_sources(client: OpenAI, conn: sqlite3.Connection,
    ...
    for i, sq in enumerate(plan.get("search_queries", [])):
        ...
        # Ask LLM to suggest URLs for this query
        url_raw = chat(client,
                       [{"role": "system", "content": SEARCH_SYS},
                        {"role": "user", "content": f"Topic: {plan.get('topic_refined', '')}\nQuery: {query}"}],
                       json_mode=True)
        try:
            urls = json.loads(url_raw).get("urls", [])
        except json.JSONDecodeError:
            urls = []
        ...
        for url in urls[:2]:
            ...
            # Extract a title from the content (first non-empty line or first 100 chars)
...
# ─── Stage 4: Adversarial Fact-Check ────────────────────────────────────────
...
def fact_check(client: OpenAI, topic: str, report: str,
               sources: list[dict]) -> dict:
    section("Adversarial Fact-Check")
    source_list = "\n".join(
...
def research_topic(client: OpenAI, conn: sqlite3.Connection,
    ...
    conn.execute("UPDATE research SET status='gathering' WHERE id=?",
                 (research_id,))
    conn.commit()
    sources = gather_sources(client, conn, research_id, plan)
    ...
    conn.execute("UPDATE research SET status='fact_checking' WHERE id=?",
                 (research_id,))
    conn.commit()
    fc = fact_check(client, topic, report, sources)
...
def main() -> None:
    global MODEL
    p = argparse.ArgumentParser(description="Research agent with fact-checking")
