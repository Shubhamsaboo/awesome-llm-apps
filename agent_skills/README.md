...
| [🚌 harness-agent-builder](harness-agent-builder/) | Scaffolds durable backend agents on an engine of reusable workers: a deterministic generator emits the turn-loop consumer wiring (send, completed-turn binding, cron or HTTP trigger, runtime model resolution), plus field-tested wire contracts and gotchas for debugging |
