...
The census also separates what *isn't* dead: **finished** projects (deployed, pushed, documented; silent because they're done, not abandoned) and **unversioned** folders (a `package.json` but no git: died before their first commit, so no history and no autopsy).
