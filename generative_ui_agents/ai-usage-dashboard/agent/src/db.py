...
DB_PATH = Path(__file__).parent.parent / "data" / "usage_dashboard.db"
...
def get_schema_context() -> str:
    ...
    for table in tables:
        ...
        if check_constraints and check_constraints["sql"]:
            sql_text = check_constraints["sql"]
            checks = re.findall(r"CHECK\(([^)]+)\)", sql_text)
            if checks:
...
def validate_write_sql(sql: str) -> str | None:
    ...
    if first_word in BLOCKED_STATEMENTS:
        return f"Statement type '{first_word}' is not allowed."
    ...
    if first_word == "UPDATE" and "WHERE" not in normalized:
        return "UPDATE without WHERE clause is not allowed."
...
def get_row_by_pk(table: str, pk_value) -> dict | None:
    allowed_tables = {"plans", "account", "usage_events", "entitlements", "invoices", "alerts"}
    ...
    conn = get_read_connection()
    try:
        row = conn.execute(f"SELECT * FROM {table} WHERE {pk_col} = ?", (pk_value,)).fetchone()
