...
def get_env_vars():
    env_vars = API_KEYS.copy()
    
    if not env_vars["FIRECRAWL_API_KEY"].startswith("fc-"):
        raise ValueError("Invalid Firecrawl API key format. Should start with 'fc-'")
    
    if not env_vars["QDRANT_URL"].startswith(('http://', 'https://')):
        env_vars["QDRANT_URL"] = f"https://{env_vars['QDRANT_URL']}"
    
    if not env_vars["QDRANT_API_KEY"].count('.') == 2:
        raise ValueError("Invalid Qdrant API key format. Should be a JWT token with two dots")
    
    if not env_vars["OPENAI_API_KEY"].startswith(("sk-", "sk-proj-")):
        raise ValueError("Invalid OpenAI API key format. Should start with 'sk-' or 'sk-proj-'")
    
    print("\nAPI Key Validation:")
    print("✓ Firecrawl API Key: [VALID]")
    print(f"✓ Qdrant URL: {env_vars['QDRANT_URL']}")
