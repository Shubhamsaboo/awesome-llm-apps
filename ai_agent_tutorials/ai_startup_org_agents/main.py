...
class AnalyzeStartupTool(BaseTool):
    ...
    def run(self):
        """Run startup analysis and store results in shared state"""
        if self._shared_state.get("startup_analysis", None) is not None:
            raise ValueError("Analysis already exists. Please proceed with strategic decisions.")
        
        startup_analysis = {
            "market_potential": "high",  # LLM will determine this
            "technical_complexity": "medium",  # LLM will determine this
            "resource_requirements": "medium",  # LLM will determine this
            "rationale": f"Analysis for startup idea: {self.startup_idea} targeting {self.target_market}"  # LLM will provide rationale
        }
        
        self._shared_state.set("startup_analysis", startup_analysis)
        return "Startup idea analyzed. Please proceed with strategic decisions."

class MakeStrategicDecision(BaseTool):
    """Tool for CEO to make and track strategic decisions"""
    decision: str = Field(..., description="The strategic decision to be made")
    decision_type: Literal["product", "technical", "marketing", "financial"] = Field(
        ..., 
    ...
    def run(self):
        ...
        if not self._shared_state.get("startup_analysis"):
            raise ValueError("Please analyze the startup idea first using AnalyzeStartupTool")
        
        decision_record = {
            "decision": self.decision,
            "type": self.decision_type,
            "based_on_analysis": self._shared_state.get("startup_analysis")
        }
        
        decisions = self._shared_state.get("strategic_decisions", [])
        decisions.append(decision_record)
        self._shared_state.set("strategic_decisions", decisions)
...
def main():
    ...
    with st.form("startup_form"):
        st.session_state.startup_idea = st.text_area("Describe your startup idea in a sentence or two")
        st.session_state.target_audience = st.text_area("Who is your target audience?")
        st.session_state.goals = st.text_area("What are your business goals?")
        st.session_state.technical_requirements = st.text_area("Any specific technical requirements? (optional)")
        st.session_state.marketing_focus = st.text_area("Any specific marketing focus? (optional)")
        submitted = st.form_submit_button("Get Comprehensive Analysis")
        
        if submitted and st.session_state.startup_idea and st.session_state.target_audience and st.session_state.goals:
            query = {
                "startup_idea": st.session_state.startup_idea,
                "target_audience": st.session_state.target_audience,
                "business_goals": st.session_state.goals,
                "technical_requirements": st.session_state.technical_requirements,
                "marketing_focus": st.session_state.marketing_focus
            }
