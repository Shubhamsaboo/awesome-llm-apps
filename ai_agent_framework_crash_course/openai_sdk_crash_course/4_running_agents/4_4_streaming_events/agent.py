...
async def advanced_streaming_example():
    ...
    print(
        f"- Final story length: {sum(len(chunk) for chunk in chunks_received)} characters"
    )
...
async def custom_streaming_processing():
    ...
    async for event in Runner.run_streamed(
        root_agent, "Explain the benefits and challenges of renewable energy in detail."
    ):
    ...
    print(
        f"- Average chunk size: {sum(analytics['chunk_sizes']) / len(analytics['chunk_sizes']):.1f} chars"
    )

    if analytics["words_per_second"]:
        avg_wps = sum(analytics["words_per_second"]) / len(
            analytics["words_per_second"]
        )
...
async def streaming_with_error_handling():
    ...
    try:
        ...
        async for event in Runner.run_streamed(
            root_agent, "What are the top 3 programming languages and why?"
        ):
            try:
                if hasattr(event, "content") and event.content:
