...
def main():
    ...
    # Initialize agents
    execution_agent, conversation_agent, config_agent, streaming_agent = (
        initialize_agents()
    )
    ...
    if demo_type == "Execution Methods":
        ...
        render_conversation_management(
            conversation_agent, model_choice, temperature, max_turns
        )
...
def render_execution_methods(agent, model_choice, temperature, max_turns):
    ...
    with col1:
        ...
        with st.form("sync_form"):
            sync_input = st.text_area(
                "Your message:",
                key="sync_input",
                value="Explain synchronous execution in simple terms",
            )
    ...
    with col2:
        ...
        with st.form("async_form"):
            async_input = st.text_area(
                "Your message:",
                key="async_input",
                value="Explain asynchronous execution benefits",
            )
    ...
    with col3:
        ...
        with st.form("streaming_form"):
            streaming_input = st.text_area(
                "Your message:",
                key="streaming_input",
                value="Write a detailed explanation of streaming execution",
            )
                ...
                try:
                    ...
                    async def stream_response():
                        ...
                        progress_container.success(
                            f"✅ Streaming completed in {execution_time:.2f}s"
                        )
...
def render_conversation_management(agent, model_choice, temperature, max_turns):
    ...
    st.markdown(
        "Compare manual conversation threading vs automatic session management."
    )
        ...
        if st.button("📋 Show Manual History"):
            if st.session_state.manual_conversation:
                ...
                for i, item in enumerate(st.session_state.manual_conversation, 1):
                    ...
                    st.write(
                        f"{i}. {role_emoji} **{item['role'].title()}:** {item['content']}"
                    )
            else:
            ...
            if session_submitted and session_input:
                with st.spinner("Processing..."):
                    try:
                        ...
                        result = asyncio.run(
                            Runner.run(agent, session_input, session=session)
                        )
...
def render_run_configuration(agent, model_choice, temperature, max_turns):
    ...
    with col1:
        ...
        with st.form("basic_config_form"):
            ...
            config_temperature = st.slider(
                "Temperature", 0.0, 2.0, 0.1, 0.1, key="config_temp"
            )
            config_top_p = st.slider("Top P", 0.0, 1.0, 0.9, 0.1, key="config_top_p")
            config_max_turns = st.number_input(
                "Max Turns", 1, 20, 5, key="config_turns"
            )

            config_input = st.text_area(
                "Your message:", value="Explain the weather in exactly 3 sentences."
            )
            ...
            if config_submitted and config_input:
                with st.spinner("Processing with configuration..."):
                    try:
                        ...
                        start_time = time.time()
                        result = asyncio.run(
                            Runner.run(agent, config_input, run_config=run_config)
                        )
                        execution_time = time.time() - start_time
    ...
    with col2:
        ...
        with st.form("tracing_config_form"):
            ...
            tracing_input = st.text_area(
                "Your message:", value="What are the benefits of structured logging?"
            )
            ...
            if tracing_submitted and tracing_input:
                with st.spinner("Processing with tracing..."):
                    try:
                        ...
                        start_time = time.time()
                        result = asyncio.run(
                            Runner.run(agent, tracing_input, run_config=run_config)
                        )
                        execution_time = time.time() - start_time

                        st.success(
                            f"✅ Completed with tracing in {execution_time:.2f}s"
                        )
                        st.write("**Response:**")
                        st.write(result.final_output)
...
def render_streaming_events(agent, model_choice, temperature, max_turns):
    ...
    st.markdown(
        "Demonstrates advanced streaming event processing and real-time analytics."
    )
        ...
        with st.form("streaming_basic_form"):
            ...
            if streaming_submitted and streaming_input:
                ...
                try:
                    ...
                    st.success(
                        f"✅ Streaming completed! {events_count} events in {final_time:.2f}s"
                    )
                    ...
                    async def process_analytics_streaming():
                        async for event in Runner.run_streamed(agent, analytics_input):
                            ...
                            if hasattr(event, "content") and event.content:
                                ...
                                with metrics_col2:
                                    avg_chunk_size = sum(
                                        analytics["chunk_sizes"]
                                    ) / len(analytics["chunk_sizes"])
                                    st.metric(
                                        "Avg Chunk Size", f"{avg_chunk_size:.1f} chars"
                                    )
                                ...
                                with metrics_col3:
                                    ...
                                    if elapsed > 0:
                                        chars_per_sec = (
                                            len(analytics["content"]) / elapsed
                                        )
                                        st.metric(
                                            "Speed", f"{chars_per_sec:.1f} chars/s"
                                        )
...
def render_exception_handling(agent, model_choice, temperature, max_turns):
    ...
    st.markdown(
        "Demonstrates proper exception handling for different SDK error scenarios."
    )
        ...
        with st.form("maxturns_form"):
            ...
            if maxturns_submitted and maxturns_input:
                try:
                    ...
                    result = asyncio.run(
                        Runner.run(agent, maxturns_input, run_config=run_config)
                    )
                    ...
                    st.info(
                        "This is expected when max_turns is set too low for complex conversations."
                    )
    ...
    with col2:
        ...
        with st.form("exception_form"):
            exception_input = st.text_area(
                "Your message:", value="Tell me about artificial intelligence"
            )
            ...
            if exception_submitted and exception_input:
                try:
                    ...
                    st.info(
                        "The model produced unexpected output. Try rephrasing your request."
                    )

                except UserError as e:
                    st.error(f"👤 User error: {e}")
                    st.info(
                        "There's an issue with the request. Please check your input."
                    )
