...
async def basic_automatic_tracing():
    ...
    result = await Runner.run(
        root_agent, "Explain what tracing means in software development."
    )
...
async def multiple_separate_traces():
    ...
    result1 = await Runner.run(
        root_agent, "What are the benefits of monitoring software?"
    )
    print(f"Trace 1 ID: {result1.run_id}")

    # Second trace (separate from first)
    result2 = await Runner.run(root_agent, "How do you debug performance issues?")
...
async def trace_structure_demo():
    ...
    result = await Runner.run(
        root_agent, "List 3 key components of observability and explain each briefly."
    )
...
async def tracing_configuration():
    ...
    result_with_trace = await Runner.run(
        root_agent, "This run will be traced normally."
    )
