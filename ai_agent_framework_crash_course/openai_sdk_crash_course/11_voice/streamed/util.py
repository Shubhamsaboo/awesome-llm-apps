...
def record_audio(
    duration: float = 5.0, sample_rate: int = 24000, channels: int = 1, dtype=np.int16
) -> np.ndarray:
    """
...
def create_silence(
    duration: float = 1.0, sample_rate: int = 24000, dtype=np.int16
) -> np.ndarray:
    """
...
def save_audio(audio_data: np.ndarray, filename: str, sample_rate: int = 24000):
    ...
    try:
        ...
        print(
            "❌ soundfile package required for saving audio. Install with: pip install soundfile"
        )
    except Exception as e:
    ...
    try:
        ...
        if original_sr != sample_rate:
            ...
            audio_data = librosa.resample(
                audio_data, orig_sr=original_sr, target_sr=sample_rate
            )
