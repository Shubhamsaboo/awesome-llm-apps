...
class ProductInfo(BaseModel):
    ...
    price_mentioned: Optional[str] = Field(
        description="Price if mentioned in review", default=None
    )
...
class ReviewMetrics(BaseModel):
    ...
    confidence_score: float = Field(
        description="Confidence in sentiment analysis (0-1)", ge=0, le=1
    )
...
class ReviewAspects(BaseModel):
    ...
    quality: Optional[str] = Field(
        description="Quality assessment if mentioned", default=None
    )
    value_for_money: Optional[str] = Field(
        description="Value assessment if mentioned", default=None
    )
    shipping: Optional[str] = Field(
        description="Shipping experience if mentioned", default=None
    )
    customer_service: Optional[str] = Field(
        description="Customer service experience if mentioned", default=None
    )
    ease_of_use: Optional[str] = Field(
        description="Usability assessment if mentioned", default=None
    )
...
class ProductReview(BaseModel):
    ...
    key_phrases: List[str] = Field(
        description="Important phrases from the review", default=[]
    )
...
def demonstrate_review_analysis():
    ...
    for i, test_case in enumerate(test_reviews, 1):
        ...
        try:
            ...
            print(
                f"😊 Sentiment: {analysis.metrics.sentiment.value.replace('_', ' ').title()}"
            )
            ...
            if analysis.aspects.customer_service:
                aspects_mentioned.append(
                    f"Service: {analysis.aspects.customer_service}"
                )
