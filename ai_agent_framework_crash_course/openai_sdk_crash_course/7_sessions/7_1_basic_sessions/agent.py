...
async def in_memory_session_example():
    ...
    result = await Runner.run(
        root_agent, "My name is Alice and I live in San Francisco.", session=session
    )
    print(f"Turn 1: {result.final_output}")

    # Second turn - agent remembers automatically
    result = await Runner.run(root_agent, "What city do I live in?", session=session)
...
async def persistent_session_example():
    ...
    # First conversation
    result = await Runner.run(
        root_agent, "I'm a software developer working on AI projects.", session=session
    )
...
async def multi_turn_conversation():
    ...
    print("🌉 First turn:")
    result = await Runner.run(
        root_agent, "What city is the Golden Gate Bridge in?", session=session
    )
    ...
    result = await Runner.run(
        root_agent, "What's the population of that state?", session=session
    )
