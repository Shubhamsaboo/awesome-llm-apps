...
def main():
    ...
    with st.sidebar:
        ...
        if demo_type == "Basic Sessions":
            session_type = st.radio("Session Type", ["In-Memory", "Persistent"])
        ...
        if st.button("🗑️ Clear All Sessions"):
            with st.spinner("Clearing sessions..."):
                for session_id in list(
                    ...
                    asyncio.run(
                        st.session_state.session_manager.clear_session(session_id)
                    )
...
def render_basic_sessions(agent):
    ...
    st.markdown(
        "Demonstrates fundamental session memory with automatic conversation history."
    )
    ...
    with col1:
        ...
        if st.button("📋 Show Conversation", key="show_in_memory"):
            items = asyncio.run(
                st.session_state.session_manager.get_session_items(session_id)
            )
            if items:
...
def render_memory_operations(agent):
    ...
    st.markdown(
        "Demonstrates advanced session memory operations including item manipulation and corrections."
    )
    ...
    with col1:
        ...
        if st.button("🔍 Get All Items"):
            ...
            if items:
                ...
                for i, item in enumerate(items, 1):
                    ...
                    st.write(
                        f"{i}. {role_emoji} **{item['role'].title()}:** {content_preview}"
                    )
            else:
        ...
        if st.button("📋 Get Recent Items"):
            items = asyncio.run(
                st.session_state.session_manager.get_session_items(
    ...
    with col2:
        ...
        with st.form("add_items_form"):
            ...
            if add_submitted and user_content and assistant_content:
                ...
                asyncio.run(
                    st.session_state.session_manager.add_custom_items(
                        session_id, custom_items
                    )
                )
        ...
        if st.button("↶ Undo Last Response"):
            popped_item = asyncio.run(
                st.session_state.session_manager.pop_last_item(session_id)
            )
            if popped_item:
                st.success(
                    f"Removed: {popped_item['role']} - {popped_item['content'][:50]}..."
                )
            else:
...
def render_multi_sessions(support_agent, sales_agent):
    ...
    st.markdown(
        "Demonstrates managing multiple conversations and different agent contexts."
    )

    tab1, tab2, tab3 = st.tabs(
        ["👤 Multi-User", "🏢 Context-Based", "🔄 Agent Handoff"]
    )
            ...
            with st.form("alice_form"):
                ...
                if alice_submitted and alice_input:
                    with st.spinner("Processing Alice's message..."):
                        session = st.session_state.session_manager.get_session(
                            alice_session_id, "multi_user.db"
                        )
                        result = asyncio.run(
                            Runner.run(support_agent, alice_input, session=session)
                        )
                        st.write(f"**Support:** {result.final_output}")

            if st.button("📋 Alice's History", key="alice_history"):
                items = asyncio.run(
                    st.session_state.session_manager.get_session_items(alice_session_id)
                )
                for item in items:
                    role_emoji = "👩" if item["role"] == "user" else "🛠️"
                    st.write(
                        f"{role_emoji} **{item['role'].title()}:** {item['content']}"
                    )
            ...
            with st.form("bob_form"):
                ...
                if bob_submitted and bob_input:
                    with st.spinner("Processing Bob's message..."):
                        session = st.session_state.session_manager.get_session(
                            bob_session_id, "multi_user.db"
                        )
                        result = asyncio.run(
                            Runner.run(support_agent, bob_input, session=session)
                        )
                        st.write(f"**Support:** {result.final_output}")

            if st.button("📋 Bob's History", key="bob_history"):
                items = asyncio.run(
                    st.session_state.session_manager.get_session_items(bob_session_id)
                )
                for item in items:
                    role_emoji = "👨" if item["role"] == "user" else "🛠️"
                    st.write(
        ...
        with col1:
            ...
            with st.form("support_context_form"):
                support_input = st.text_input(
                    "Support question:", key="support_context_input"
                )
                ...
                if support_submitted and support_input:
                    with st.spinner("Processing support question..."):
                        session = st.session_state.session_manager.get_session(
                            support_session_id, "contexts.db"
                        )
                        result = asyncio.run(
                            Runner.run(support_agent, support_input, session=session)
                        )
                        st.write(f"**Support:** {result.final_output}")
            ...
            with st.form("sales_context_form"):
                ...
                if sales_submitted and sales_input:
                    with st.spinner("Processing sales inquiry..."):
                        session = st.session_state.session_manager.get_session(
                            sales_session_id, "contexts.db"
                        )
                        result = asyncio.run(
                            Runner.run(sales_agent, sales_input, session=session)
                        )
    ...
    with tab3:
        ...
        selected_agent = st.radio(
            "Select Agent:", ["Sales Agent", "Support Agent"], horizontal=True
        )
        ...
        with st.form("handoff_form"):
            ...
            if handoff_submitted and handoff_input:
                with st.spinner(f"Processing with {selected_agent}..."):
                    session = st.session_state.session_manager.get_session(
                        shared_session_id, "shared.db"
                    )
                    result = asyncio.run(
                        Runner.run(agent, handoff_input, session=session)
                    )
        ...
        if st.button("📋 Show Shared Conversation"):
            items = asyncio.run(
                st.session_state.session_manager.get_session_items(shared_session_id)
            )
            if items:
                ...
                for i, item in enumerate(items, 1):
                    if item["role"] == "user":
                        ...
                        agent_emoji = (
                            "💰"
                            if "sales" in item["content"].lower()
                            or "price" in item["content"].lower()
                            else "🛠️"
                        )
