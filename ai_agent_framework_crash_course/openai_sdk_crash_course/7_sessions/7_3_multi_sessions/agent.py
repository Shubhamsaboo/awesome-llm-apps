...
async def multi_user_sessions():
    ...
    result = await Runner.run(
        support_agent, "I forgot my password", session=alice_session
    )
    print("Alice: I forgot my password")
    print(f"Support: {result.final_output}")

    result = await Runner.run(
        support_agent, "My email is alice@example.com", session=alice_session
    )
    print("Alice: My email is alice@example.com")
    print(f"Support: {result.final_output}")

    # Bob's separate conversation
    print("\nBob's conversation:")
    result = await Runner.run(
        support_agent, "My app keeps crashing", session=bob_session
    )
    print("Bob: My app keeps crashing")
    print(f"Support: {result.final_output}")

    # Alice continues her conversation (agent remembers her context)
    print("\nAlice continues:")
    result = await Runner.run(
        support_agent, "Did you find my account?", session=alice_session
    )
    print("Alice: Did you find my account?")
    print(f"Support: {result.final_output}")
...
async def context_based_sessions():
    ...
    result = await Runner.run(
        support_agent, "I can't access my premium features", session=support_session
    )
    print("Customer: I can't access my premium features")
    print(f"Support: {result.final_output}")

    # Sales conversation
    print("\nSales context:")
    result = await Runner.run(
        sales_agent, "What premium features do you offer?", session=sales_session
    )
    print("Prospect: What premium features do you offer?")
    print(f"Sales: {result.final_output}")

    # Continue support conversation
    print("\nBack to support:")
    result = await Runner.run(
        support_agent, "I'm on the premium plan", session=support_session
    )
    print("Customer: I'm on the premium plan")
    print(f"Support: {result.final_output}")
...
async def shared_session_agents():
    ...
    print(
        "Customer: I'm interested in your premium plan but have technical questions."
    )
    print(f"Sales: {result.final_output}")
    ...
    print("Customer: Can you help me understand the technical requirements?")
    print(f"Support: {result.final_output}")
    ...
    print("Customer: Thanks for the technical info. How do I upgrade?")
    print(f"Sales: {result.final_output}")
...
async def session_organization():
    ...
    result = await Runner.run(
        support_agent, "Daily check-in", session=user_daily_session
    )
    print(f"Response: {result.final_output}")

    print("\nFeature-specific chat:")
    result = await Runner.run(
        support_agent, "Chat feature question", session=chat_session
    )
    print(f"Response: {result.final_output}")

    print("\nThread-based conversation:")
    result = await Runner.run(
        support_agent, "Thread conversation", session=thread_session
    )
    print(f"Response: {result.final_output}")
