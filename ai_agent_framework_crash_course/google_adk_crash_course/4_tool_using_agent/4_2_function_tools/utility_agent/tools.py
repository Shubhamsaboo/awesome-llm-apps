...
def process_text(text: str, operation: str) -> Dict[str, Union[str, int]]:
    ...
    try:
        ...
        operations = {
            ...
            "extract_emails": lambda t: {
                "emails": re.findall(
                    r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b", t
                ),
                "original": t,
            },
            "extract_urls": lambda t: {
                "urls": re.findall(
            ...
            "word_frequency": lambda t: {
                "word_frequency": dict(
                    sorted(
                        [
                            (word.lower(), t.lower().split().count(word.lower()))
        ...
        return {"error": f"Error processing text: {str(e)}", "status": "error"}


def format_datetime(
    date_input: str, input_format: str, output_format: str
...
def calculate_date_difference(
    date1: str, date2: str, date_format: str
    ...
    try:
        ...
        return {
            "difference": {
                ...
                "breakdown": {"years": years, "months": months, "days": remaining_days},
            },
            "date1": date1,
            "date2": date2,
...
def generate_uuid(version: int = 4) -> Dict[str, Union[str, int]]:
    ...
    try:
        if version == 1:
            ...
            return {"error": "Invalid UUID version. Use 1, 4, or 5", "status": "error"}
        ...
        return {"error": f"Error generating UUID: {str(e)}", "status": "error"}
...
def hash_text(text: str, algorithm: str = "sha256") -> Dict[str, Union[str, Dict]]:
    ...
    try:
        ...
        return {"error": f"Error hashing text: {str(e)}", "status": "error"}
...
def encode_decode_base64(text: str, operation: str) -> Dict[str, Union[str, int]]:
    ...
    try:
        ...
        return {"error": f"Error in Base64 operation: {str(e)}", "status": "error"}
...
def format_json(json_string: str, indent: int = 2) -> Dict[str, Union[str, Dict]]:
    ...
    try:
        ...
        return {"error": f"Error formatting JSON: {str(e)}", "status": "error"}
