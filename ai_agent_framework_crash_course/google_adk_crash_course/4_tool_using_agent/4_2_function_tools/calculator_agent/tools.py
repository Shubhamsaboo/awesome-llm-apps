...
def calculate_basic_math(expression: str) -> Dict[str, Union[float, str]]:
    ...
    try:
        ...
        if not safe_expression.strip():
            return {"error": "Empty or invalid expression", "status": "error"}
...
def convert_temperature(
    ...
    try:
        ...
        return {"error": f"Error converting temperature: {str(e)}", "status": "error"}


def calculate_compound_interest(
    principal: float, rate: float, years: int, compound_frequency: int = 1
) -> Dict[str, Union[float, str, Dict]]:
    """
...
def calculate_percentage(value: float, total: float) -> Dict[str, Union[float, str]]:
    ...
    try:
        ...
        return {"error": f"Error calculating percentage: {str(e)}", "status": "error"}
...
def calculate_statistics(numbers: List[float]) -> Dict[str, Union[float, str, int]]:
    ...
    try:
        if not numbers:
            return {
                "error": "Cannot calculate statistics for empty list",
                "status": "error",
            }
        ...
        return {"error": f"Error calculating statistics: {str(e)}", "status": "error"}


def round_number(
    number: float, decimal_places: int = 2
    ...
    try:
        ...
        return {"error": f"Error rounding number: {str(e)}", "status": "error"}
