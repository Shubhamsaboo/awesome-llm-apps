...
class SimplePlugin(BasePlugin):
    ...
    async def on_user_message_callback(
        ...
        modified_parts = [
            types.Part(text=f"[{timestamp}] {part.text}")
    ...
    async def after_run_callback(self, *, invocation_context) -> None:
        print(
            f"📊 [Plugin] Final Report: {self.agent_count} agents, {self.tool_count} tools"
        )
...
async def calculator_tool(
    tool_context: ToolContext, operation: str, a: float, b: float
) -> Dict[str, Any]:
...
runner = InMemoryRunner(
    agent=agent, app_name="plugin_demo_app", plugins=[SimplePlugin()]
)
...
async def run_agent(message: str) -> str:
    ...
    session = await session_service.get_session(
        app_name="plugin_demo_app", user_id=user_id, session_id=session_id
    )
    if not session:
