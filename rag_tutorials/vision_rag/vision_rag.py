...
def process_pdf_file(
    pdf_file, cohere_client, base_output_folder="pdf_pages"
    ...
    try:
        ...
        for i, page in enumerate(doc.pages()):
            ...
            pix = page.get_pixmap(
                dpi=150
            )  # Adjust DPI as needed for quality/performance
            ...
            if emb is not None:
                ...
                st.warning(
                    f"Could not embed page {page_num} from {pdf_filename}. Skipping."
                )
        ...
        valid_paths = [
            ...
            if page_embeddings[i] is not None
        ]
        ...
        if img_path not in st.session_state.image_paths:
            try:
                ...
                if file_type == "application/pdf":
                    ...
                    pdf_page_paths, pdf_page_embeddings = process_pdf_file(
                        uploaded_file, cohere_client=co
                    )
                    if pdf_page_paths and pdf_page_embeddings:
                        ...
                        unique_new_paths = [
                            p for p in pdf_page_paths if p not in current_paths_set
                        ]
                        if unique_new_paths:
                            indices_to_add = [
                                i
                                for i, p in enumerate(pdf_page_paths)
                                if p in unique_new_paths
                            ]
                            newly_uploaded_paths.extend(unique_new_paths)
                            newly_uploaded_embeddings.extend(
                                [pdf_page_embeddings[idx] for idx in indices_to_add]
                            )
