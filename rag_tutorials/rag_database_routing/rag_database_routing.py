...
def route_query(question: str) -> Optional[DatabaseType]:
    ...
    try:
        ...
        if best_score >= confidence_threshold and best_db_type:
            st.success(
                f"Using vector similarity routing: {best_db_type} (confidence: {best_score:.3f})"
            )
            return best_db_type

        st.warning(
            f"Low confidence scores (below {confidence_threshold}), falling back to LLM routing"
        )
    ...
    try:
        ...
        if relevant_docs:
            ...
            combine_docs_chain = create_stuff_documents_chain(
                st.session_state.llm, retrieval_qa_prompt
            )
...
def _handle_web_fallback(question: str) -> tuple[str, list]:
    ...
    with st.spinner("Researching..."):
        agent_input = {
            "messages": [
                HumanMessage(
                    content=f"Research and provide a detailed answer for: '{question}'"
                )
            ],
            "is_last_step": False,
        }
        ...
        try:
            response = fallback_agent.invoke(
                agent_input, config={"recursion_limit": 100}
            )
...
def main():
    ...
    st.info(
        "Upload documents to populate the databases. Each tab corresponds to a different database."
    )
    tabs = st.tabs(
        [collection_config.name for collection_config in COLLECTIONS.values()]
    )
