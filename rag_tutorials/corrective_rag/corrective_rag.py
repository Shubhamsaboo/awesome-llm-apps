...
def setup_sidebar():
    ...
    with st.sidebar:
        ...
        st.session_state.doc_url = st.text_input(
            "Document URL", value=st.session_state.doc_url
        )
    ...
    try:
        ...
        try:
            ...
            progress_placeholder.error(
                f"Search failed after retries: {str(search_error)}"
            )
            return {"keys": {"documents": documents, "question": question}}
        ...
        progress_placeholder.success(
            f"Successfully added {len(web_results)} search results"
        )
...
st.text(
    "A possible query: What are the experiment results and ablation studies in this research paper?"
)
