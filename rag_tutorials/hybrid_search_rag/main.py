...
def initialize_config(
    openai_key: str, anthropic_key: str, cohere_key: str, db_url: str
) -> RAGLiteConfig:
...
def handle_fallback(query: str) -> str:
    try:
        client = anthropic.Anthropic(
            api_key=st.session_state.user_env["ANTHROPIC_API_KEY"]
        )
...
def main():
    st.set_page_config(
        page_title="LLM-Powered Hybrid Search-RAG Assistant", layout="wide"
    )
    ...
    for state_var in ["chat_history", "documents_loaded", "my_config", "user_env"]:
        if state_var not in st.session_state:
            st.session_state[state_var] = (
                ...
                if state_var == "documents_loaded"
                else None
                if state_var == "my_config"
                else {}
            )
    ...
    with st.sidebar:
        ...
        cohere_key = st.text_input(
            "Cohere API Key",
            value=st.session_state.get("cohere_key", ""),
            type="password",
            placeholder="Enter Cohere key",
        )
        db_url = st.text_input(
            "Database URL",
            value=st.session_state.get("db_url", "sqlite:///raglite.sqlite"),
    ...
    if st.session_state.documents_loaded:
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    if not reranked_chunks or len(reranked_chunks) == 0:
                        logger.info(
                            "No relevant documents found. Falling back to Claude."
                        )
                        st.info(
        ...
        st.info(
            "Please configure your API keys and upload documents to get started."
