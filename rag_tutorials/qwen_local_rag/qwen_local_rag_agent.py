...
st.info(
    "**Qwen3:** The latest generation of large language models in Qwen series, offering a comprehensive suite of dense and mixture-of-experts (MoE) models."
)
st.info(
    "**Gemma 3:** These models are multimodal—processing text and images—and feature a 128K context window with support for over 140 languages."
)
# -------------------------
...
if st.session_state.rag_enabled:
    ...
    with st.expander("📁 Upload Documents or URLs for RAG", expanded=False):
        if not qdrant_client:
            st.warning(
                "⚠️ Please configure Qdrant API Key and URL in the sidebar to enable document processing."
            )
        else:
    ...
    if st.session_state.rag_enabled:
        ...
        if not st.session_state.force_web_search and st.session_state.vector_store:
            ...
            if docs:
                ...
                st.info(
                    "🔄 No relevant documents found in database, falling back to web search..."
                )
...
Please provide a comprehensive answer based on the available information."""
                    ...
                    st.info(
                        "ℹ️ No relevant information found in documents or web search."
                    )
                ...
                with st.chat_message("assistant"):
                    ...
                    if (
                        not st.session_state.force_web_search
                        and "docs" in locals()
                        and docs
                    ):
                        with st.expander("🔍 See document sources"):
                            for i, doc in enumerate(docs, 1):
                                ...
                                st.write(
                                    f"{source_icon} Source {i} from {source_name}:"
                                )
...
Please provide a comprehensive answer based on the available information."""
    ...
    st.warning(
        "You can directly talk to qwen and gemma models locally! Toggle the RAG mode to upload documents!"
    )
