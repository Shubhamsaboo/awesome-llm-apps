...
def main():
    st.set_page_config(
        page_title="Local LLM-Powered Hybrid Search-RAG Assistant", layout="wide"
    )
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    if not reranked_chunks or len(reranked_chunks) == 0:
                        logger.info(
                            "No relevant documents found. Falling back to local LLM."
                        )
                        with st.spinner("Using general knowledge to answer..."):
                            ...
                            if full_response.startswith("I apologize"):
                                st.warning(
                                    "No relevant documents found and fallback failed."
                                )
                            else:
