...
# 注意：这些消息需要与 __001__langgraph_fastapi.py 中的 NODE_PROGRESS_MAP 保持同步。
...
def query_tcm_knowledge(user_input: str, messages: list = None) -> dict:
    """
    调用 FastAPI /process 端点进行中医知识查询或小红书内容生成。

    LangGraph 工作流会自动判断用户意图并路由：
      - 小红书发布意图 → 文案生成 → 图片生成 → 发布
      - 中医相关问题 → 实体抽取 → FAISS 匹配 → Cypher 查询 → 回答
      - 非中医问题 → LLM 直接回答
