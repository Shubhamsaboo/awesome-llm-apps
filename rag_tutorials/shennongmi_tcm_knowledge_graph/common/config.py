...
class Config:
    ...
    Attributes:
        ...
        JIMENG_AK (str): 即梦 AI Access Key
        JIMENG_SK (str): 即梦 AI Secret Key
    """
    ...
    def __init__(self):
        ...
        # ========== FAISS 向量索引路径 ==========
        # 这两个文件由 __003__create_neo4j_database 流程生成，
        # 用于基于向量相似度的实体检索
        self.ENTITY_INDEX_PATH = get_file_path("__003__create_neo4j_database/neo4j_embedding_faiss.index")
        self.ENTITY_ID2TEXT_PATH = get_file_path("__003__create_neo4j_database/neo4j_embedding_faiss_id2text.pkl")

        # ========== 即梦 AI 相关 ==========
        # 即梦（字节跳动旗下 AI 图片生成服务）的 API 凭证
        self.JIMENG_AK = os.getenv("JIMENG_AK")
        self.JIMENG_SK = os.getenv("JIMENG_SK")

        # ========== TCM 知识图谱元数据 ==========
        # 🔧 修复：元数据改为惰性加载，避免 import 时文件不存在导致崩溃。
        # tcm_metadata.json 由 __002__export_metadata.py 在 Neo4j 数据导入后生成，
        # 在此之前导入本模块不应该因此失败。
    ...
    def TCM_METADATA(self) -> str:
        ...
        if self._tcm_metadata is None:
            # 首次访问：从磁盘加载并缓存
            metadata_path = get_file_path("__003__create_neo4j_database/tcm_metadata.json")
