    ...
    with st.sidebar:
        ...
        with st.form("api_credentials"):
            ...
            qdrant_key = st.text_input(
                "Qdrant API Key", type="password", help="Enter your Qdrant API key"
            )
            ...
            if st.form_submit_button("Submit Credentials"):
                try:
                    client = QdrantClient(
                        url=qdrant_url, api_key=qdrant_key, timeout=60
                    )
...
embedding = CohereEmbeddings(
    model="embed-english-v3.0", cohere_api_key=st.session_state.cohere_api_key
)
...
def create_fallback_agent(chat_model: BaseLanguageModel):
    ...
    def web_research(query: str) -> str:
        ...
        try:
            ...
            return (
                f"Search failed: {str(e)}. Providing answer based on general knowledge."
            )
    ...
    try:
        ...
        if relevant_docs:
            ...
            combine_docs_chain = create_stuff_documents_chain(
                chat_model, retrieval_qa_prompt
            )
            ...
            with st.spinner("Researching..."):
                agent_input = {
                    "messages": [
                        HumanMessage(
                            content=f"""Please thoroughly research the question: '{query}' and provide a detailed and comprehensive response. Make sure to gather the latest information from credible sources. Minimum 400 words."""
                        )
                    ],
                    "is_last_step": False,
                }
...
""",
                    ...
                    fallback_response = chat_model.invoke(
                        f"Please provide a general answer to: {query}"
                    ).content
                    return (
                        f"Web search unavailable. General response: {fallback_response}",
                        [],
                    )
...
uploaded_file = st.file_uploader(
    "Choose a PDF or Image File", type=["pdf", "jpg", "jpeg"]
)
