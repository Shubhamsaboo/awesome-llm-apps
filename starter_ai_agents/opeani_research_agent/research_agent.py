    ...
    with trace("News Research", group_id=st.session_state.conversation_id):
        ...
        try:
            ...
            if hasattr(triage_result, "new_items"):
                messages = [
                    item for item in triage_result.new_items if hasattr(item, "content")
                ]
                if messages:
                    raw_content = "\n\n".join(
                    ...
                    with message_container:
                        st.write(
                            "⚠️ **Research completed but there was an issue generating the structured report.**"
                        )
                        st.write(
                            "Raw research results are available in the Report tab."
                        )
